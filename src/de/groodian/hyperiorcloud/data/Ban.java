		...
		if (hasBan(targetUUID)) {
			try {
				...
				if (type.equals(DAYS)) {
					...
					ps.setString(4, getString("history", targetUUID) + "\n(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "d. Grund: " + reason);
				}
				if (type.equals(HOURS)) {
					ps.setString(2, dateFormatterTime.format(LocalDateTime.now().plusHours(duration)));
					ps.setString(4, getString("history", targetUUID) + "\n(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "h. Grund: " + reason);
				}
				if (type.equals(MINUTES)) {
					ps.setString(2, dateFormatterTime.format(LocalDateTime.now().plusMinutes(duration)));
					ps.setString(4, getString("history", targetUUID) + "\n(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "m. Grund: " + reason);
				}
				if (type.equals(SECONDS)) {
					ps.setString(2, dateFormatterTime.format(LocalDateTime.now().plusSeconds(duration)));
					ps.setString(4, getString("history", targetUUID) + "\n(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "s. Grund: " + reason);
				}
			...
			try {
				...
				if (type.equals(DAYS)) {
					...
					ps.setString(5, "(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "d. Grund: " + reason);
				}
				if (type.equals(HOURS)) {
					ps.setString(3, dateFormatterTime.format(LocalDateTime.now().plusHours(duration)));
					ps.setString(5, "(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "h. Grund: " + reason);
				}
				if (type.equals(MINUTES)) {
					ps.setString(3, dateFormatterTime.format(LocalDateTime.now().plusMinutes(duration)));
					ps.setString(5, "(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "m. Grund: " + reason);
				}
				if (type.equals(SECONDS)) {
					ps.setString(3, dateFormatterTime.format(LocalDateTime.now().plusSeconds(duration)));
					ps.setString(5, "(" + dateFormatterTime.format(LocalDateTime.now()) + ") TEMPBAN von " + executorName + " für " + duration + "s. Grund: " + reason);
				}
				ps.executeUpdate();
			} catch (SQLException e) {
				e.printStackTrace();
