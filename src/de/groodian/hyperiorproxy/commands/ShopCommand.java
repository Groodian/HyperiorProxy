...
public class ShopCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        sender.sendMessage("§7[§bHyperior.de§7] §6Informationen zum §eVIP-Rang§6:");
        ProxyServer.getInstance().getScheduler().schedule(plugin, new Runnable() {
            @Override
            public void run() {
                sender.sendMessage("§8§ §7Der §eVIP-Rang §7ist §6Lifetime §7nur während der §cBeta-Phase §7erhältlich.");
                ProxyServer.getInstance().getScheduler().schedule(plugin, new Runnable() {
                    @Override
                    public void run() {
                        sender.sendMessage("§8§ §7Du kannst §avolle Server beitreten §7und damit bevorzugt spielen.");
                        ProxyServer.getInstance().getScheduler().schedule(plugin, new Runnable() {
                            @Override
                            public void run() {
                                sender.sendMessage("§8§ §7Dein Name hat auf dem Server eine §eGelbe Farbe§7.");
                                ProxyServer.getInstance().getScheduler().schedule(plugin, new Runnable() {
                                    @Override
                                    public void run() {
                                        sender.sendMessage("§8§ §7Du hast auf dem Server einen §eexklusiven 'VIP' Präfix§7 vor deinem Namen.");
                                        ProxyServer.getInstance().getScheduler().schedule(plugin, new Runnable() {
                                            @Override
                                            public void run() {
