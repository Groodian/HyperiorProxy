...
public class Ban {
    ...
    public UserHistory loadUserHistory(UUID userHistoryId) {
        ...
        try {
            PreparedStatement ps = databaseConnection.getPreparedStatement(
                    "SELECT target, created_by, type, reason, created_at, duration FROM hyperior_mc.users_ban_history WHERE id = ?");
            ...
            if (rs.next()) {
                ...
                userHistory = new UserHistory(userHistoryId,
    ...
    public String getBanTimeLeft(UserHistory userHistory) {
        ...
        switch (userHistory.getType()) {
            case BAN -> {
                ...
                String timeLeftString = Time.timeLeftString(userHistoryCreatedAt.plus(duration), OffsetDateTime.now());
    ...
    public void ban(UserHistory userHistory) {
        DatabaseConnection databaseConnection = HyperiorCore.getDatabaseManager().getConnection();
    ...
    public void loadUserHistory(String name) {
